...
(defmethod grab-article ((document plump:nesting-node))
  (alexandria:when-let* ((body (clss:select "body" document))
    ...
    (flet ((remove-child* (child)
             "Remove child, if possible."
             (when (plump:parent child)
      ...
      (loop for node across (clss:select "*" body)
            ...
            when (and (plump:element-p node)
              ...
              do (cond
                   ...
                   ((and (not (has-block-children-p node))
                         (every (alexandria:disjoin #'phrasing-content-p #'whitespace-node-p)
                                (plump:children node)))
                    (replace-with-tag node "p")))
                 ;; TODO: L1005-1319
            ))
    body))
